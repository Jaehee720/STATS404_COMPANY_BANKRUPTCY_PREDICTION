...
if __name__ == '__main__':
    ...
    df = pd.read_csv(filepath_or_buffer=FILE_NAME, encoding='latin-1', nrows=6819, index_col=0)
    ...
    high_corr_columns = save_variables_high_correlation_with_bank(importances, index, COLUMNS)

    LOGGER.info("--- 4-1) Step2: Save the features that have a correlation higher with Bankrupt")
    # Subtract important X features from all the X features
    X = df.loc[:, high_corr_columns]
    print(X.shape)
    
    LOGGER.info("--- 4-1) Step3: Remove the features that have a correlation higher than 90% with other independent variables")
    ...
    LOGGER.info("--- 4-1) Step4: Save all the selected variables to new_df and Bankrupt")

    # Generate a new df with the important features
    # Save some features that I think it's important
    important_features = X['net income to total assets', 'Cash flow to total assets', 'tax Pre-net interest rate', 'inventory and accounts receivable/net value']

    # Save y feature and all important X features
    new_df = pd.concat([df['Bankrupt?'], X[important_features], X[selected_columns]], axis=1)
    ...
    LOGGER.info("--- Generate a precision recall curve to check if we can change the threshold to increase FalseNegative.")
